...
/// ```
///
/// IMPORTANT: Replace the all-zero placeholder below with real key bytes
/// before the first signed release ships. The placeholder will never match
/// any real signature, so unsigned builds will safely fall through to the
/// [`VerifyResult::Unsigned`] transition path.
const PRIMARY_PUBLIC_KEY: &[u8; 32] = &[
    // TODO: Replace with real 32-byte Ed25519 public key before production
