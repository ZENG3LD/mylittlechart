...
//!
//! Thin wrapper around `uzor_backend_vello_hybrid::VelloHybridRenderContext` that
...
use uzor_backend_vello_hybrid::VelloHybridRenderContext as InnerContext;
...
///
/// All drawing primitives are delegated to [`uzor_backend_vello_hybrid::VelloHybridRenderContext`].
...
impl<'a> UzorRenderContext for VelloHybridChartRenderContext<'a> {
    ...
    // Images — vello-hybrid backend has no image support; use default no-ops from trait
    fn draw_image_rgba(
        &mut self,
        _data: &[u8],
        _img_width: u32,
    ...
    // Blur / glass — not supported in vello-hybrid backend; use no-ops
...
impl<'a> RenderContextExt for VelloHybridChartRenderContext<'a> {
    ...
    fn set_blur_image(&mut self, image: Option<()>, width: u32, height: u32) {
