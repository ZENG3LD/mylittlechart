...
//!
//! Thin wrapper around `uzor_backend_tiny_skia::TinySkiaCpuRenderContext` that
...
use uzor_backend_tiny_skia::TinySkiaCpuRenderContext as InnerContext;
...
///
/// All drawing primitives are delegated to [`uzor_backend_tiny_skia::TinySkiaCpuRenderContext`].
...
impl<'a> UzorRenderContext for TinySkiaChartRenderContext<'a> {
    ...
    // Images — tiny_skia backend has no image support; use default no-ops from trait
    fn draw_image_rgba(
        &mut self,
        _data: &[u8],
        _img_width: u32,
    ...
    // Blur / glass — not supported in tiny_skia backend; use no-ops
...
// ─── RenderContextExt — tiny_skia backend has no blur image ──────────────────
