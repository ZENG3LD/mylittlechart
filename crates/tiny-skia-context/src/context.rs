//! TinySkia CPU RenderContext implementation
//!
//! Thin wrapper around `uzor_render_tiny_skia::TinySkiaCpuRenderContext` that
//! delegates all drawing operations to the CPU-only tiny-skia backend and adds
...
use uzor_render_tiny_skia::TinySkiaCpuRenderContext as InnerContext;
...
/// Chart render context backed by the CPU-only tiny-skia renderer.
///
/// All drawing primitives are delegated to [`uzor_render_tiny_skia::TinySkiaCpuRenderContext`].
/// This type only adds chart-domain coordinate conversion on top.
pub struct TinySkiaChartRenderContext<'a> {
    inner: InnerContext,
...
impl<'a> TinySkiaChartRenderContext<'a> {
    ...
    pub fn new(
        ...
        Self {
            inner: InnerContext::new(width, height, dpr),
            viewport,
            price_scale,
            coord_override: None,
            bars: Vec::new(),
    ...
    /// Access the inner context (e.g., to call `clear`, `pixels`, or `resize`).
...
impl<'a> Painter for TinySkiaChartRenderContext<'a> {
...
impl<'a> TextRenderer for TinySkiaChartRenderContext<'a> {
...
impl<'a> TextMetrics for TinySkiaChartRenderContext<'a> {
...
impl<'a> Masking for TinySkiaChartRenderContext<'a> {
    fn clip(&mut self) { self.inner.clip() }
}

// ─── Effects ──────────────────────────────────────────────────────────────────

impl<'a> Effects for TinySkiaChartRenderContext<'a> {
...
impl<'a> ShapeHelpers for TinySkiaChartRenderContext<'a> {
...
impl<'a> BatchPainter for TinySkiaChartRenderContext<'a> {
...
impl<'a> GradientPainter for TinySkiaChartRenderContext<'a> {
...
impl<'a> UiEffectHelpers for TinySkiaChartRenderContext<'a> {
...
impl<'a> UzorRenderContext for TinySkiaChartRenderContext<'a> {
    fn dpr(&self) -> f64 { self.inner.dpr() }
}

// ─── RenderContextExt — tiny-skia backend has no blur image ──────────────────

impl<'a> RenderContextExt for TinySkiaChartRenderContext<'a> {
    type BlurImage = ();
...
impl<'a> ChartRenderContext for TinySkiaChartRenderContext<'a> {
