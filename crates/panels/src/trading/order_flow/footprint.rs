...
impl TradingPanel for FootprintState {
    ...
    fn render(
        ...
        {
            let body_id = format!("{}:footprint:body", slot_prefix);
    ...
    fn handle_scroll(&mut self, local_id: &str, _dx: f64, dy: f64) -> bool {
        if local_id == "footprint:body" {
            FootprintState::handle_scroll(self, dy * 3.0);
            true
        } else {
            false
        }
    ...
    fn handle_drag_start(&mut self, local_id: &str, _x: f64, _y: f64) -> bool {
        local_id == "footprint:body"
    }

    fn handle_drag_move(&mut self, local_id: &str, dx: f64, dy: f64) -> bool {
        if local_id == "footprint:body" {
            FootprintState::handle_drag(self, dx, dy);
            true
        } else {
            false
        }
    ...
    fn handle_double_click(&mut self, local_id: &str, _x: f64, _y: f64) -> bool {
        if local_id == "footprint:body" {
            FootprintState::handle_double_click(self);
            true
        } else {
            false
        }
