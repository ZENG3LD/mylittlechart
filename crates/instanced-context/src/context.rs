...
impl<'a> InstancedChartRenderContext<'a> {
    ...
    ///
    /// * `screen_w / screen_h` — full surface dimensions in pixels.
    /// * `offset_x / offset_y` — canvas offset applied to all draw calls (chart rect).
    ...
    pub fn new(
        ...
        Self {
            inner: InnerContext::new(screen_w, screen_h, offset_x, offset_y),
            viewport,
            price_scale,
            coord_override: None,
        }
    ...
    /// Mutably access the inner context (e.g., to call `clear()` between frames).
...
impl<'a> RenderContextExt for InstancedChartRenderContext<'a> {
    type BlurImage = ();
    // All methods default to no-ops — instanced backend does not support blur.
}

// ─── ChartRenderContext — chart coordinate logic ──────────────────────────────

impl<'a> ChartRenderContext for InstancedChartRenderContext<'a> {
