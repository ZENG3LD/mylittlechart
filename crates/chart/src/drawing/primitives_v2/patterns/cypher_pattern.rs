...
impl Primitive for CypherPattern {
    fn type_id(&self) -> &'static str { "cypher_pattern" }
    ...
    fn set_points(&mut self, pts: &[(f64, f64)]) { for (i, &p) in pts.iter().take(5).enumerate() { self.points[i] = p; } }
    ...
    fn move_control_point(&mut self, pt: ControlPointType, bar: f64, price: f64) {
        match pt {
            ControlPointType::Index(i) if (i as usize) < 5 => self.points[i as usize] = (bar, price),
...
pub fn metadata() -> PrimitiveMetadata {
    PrimitiveMetadata {
        type_id: "cypher_pattern", display_name: "Cypher Pattern", kind: PrimitiveKind::Pattern,
        click_behavior: ClickBehavior::MultiPoint(5), tooltip: "Cypher harmonic pattern", icon: "cypher_pattern", default_color: "#673AB7",
