//! Triangle Up primitive - upward pointing triangle marker
//!
...
impl TriangleUp {
    pub fn new(bar1: f64, price1: f64, bar2: f64, price2: f64, color: &str) -> Self {
        let mut data = PrimitiveData {
            type_id: "triangle_up".to_string(),
            display_name: "Triangle Up".to_string(),
...
impl Primitive for TriangleUp {
    fn type_id(&self) -> &'static str { "triangle_up" }
    ...
    fn render(&self, ctx: &mut dyn RenderContext, is_selected: bool) {
        ...
        ctx.begin_path();
        ctx.move_to(crisp(x, dpr), crisp(y - s, dpr)); // top point
        ctx.line_to(crisp(x - s, dpr), crisp(y + s, dpr)); // bottom left
        ctx.line_to(crisp(x + s, dpr), crisp(y + s, dpr)); // bottom right
        ctx.close_path();
        ctx.fill();
        ctx.stroke();
...
pub fn metadata() -> PrimitiveMetadata {
    PrimitiveMetadata {
        type_id: "triangle_up", display_name: "Triangle Up", kind: PrimitiveKind::Signal,
        click_behavior: ClickBehavior::TwoPoint, tooltip: "Upward triangle marker", icon: "arrow_up", default_color: "#4CAF50",
        factory: |points, color| {
            let (b1, p1) = points.first().copied().unwrap_or((0.0, 0.0));
            let (b2, p2) = points.get(1).copied().unwrap_or((b1 + 2.0, p1));
            Box::new(TriangleUp::new(b1, p1, b2, p2, color))
        },
        supports_text: true,
        has_levels: false,
