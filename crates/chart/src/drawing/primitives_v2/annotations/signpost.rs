...
pub struct Signpost {
    ...
    pub bar2: f64, pub price2: f64, // Size point (determines scale)
...
impl Primitive for Signpost {
    ...
    fn hit_test(&self, sx: f64, sy: f64, vp: &Viewport, ps: &PriceScale) -> HitTestResult {
        ...
        // Check signpost body area (based on size from distance between points)
    ...
    fn render(&self, ctx: &mut dyn RenderContext, is_selected: bool) {
        ...
        // Calculate size based on distance between the two points
        let size = ((x2 - x1).powi(2) + (y2 - y1).powi(2)).sqrt().max(30.0);
        let half_height = size / 3.0; // Height is 1/3 of the total size

        ctx.set_fill_color(&self.data.color.stroke);
        ctx.set_stroke_color(&self.data.color.stroke);
        ctx.set_stroke_width(self.data.width);

        // Draw signpost shape based on direction, scaled by size
        ctx.begin_path();
        ...
        // Render text centered inside the signpost shape
        ...
        if !label.is_empty() {
            ...
            // Calculate center of the signpost body
...
pub fn metadata() -> PrimitiveMetadata {
    PrimitiveMetadata {
        ...
        factory: |points, color| { let (b1, p1) = points.first().copied().unwrap_or((0.0, 0.0)); let (b2, p2) = points.get(1).copied().unwrap_or((b1+3.0, p1)); Box::new(Signpost::new(b1, p1, b2, p2, color)) },
