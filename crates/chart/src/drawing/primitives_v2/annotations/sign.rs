//! Sign primitive - icon-based marker with scalable size
//!
...
pub enum SignType { #[default] Circle, Square, Diamond, Triangle, Star, Cross, Check, X }
...
impl Sign {
    ...
    /// Calculate the size (radius) based on distance between center and size point
    ...
    /// Calculate the size using RenderContext coordinates
...
impl Primitive for Sign {
    fn type_id(&self) -> &'static str { "sign" }
    ...
    fn hit_test(&self, sx: f64, sy: f64, vp: &Viewport, ps: &PriceScale) -> HitTestResult {
        ...
        // Check body area (circle around center with calculated size)
    ...
    fn render(&self, ctx: &mut dyn RenderContext, is_selected: bool) {
        ...
        match self.sign_type {
            ...
            SignType::Square => {
                ctx.fill_rect(crisp(x - s, dpr), crisp(y - s, dpr), size, size);
            }
            SignType::Diamond => {
            ...
            SignType::Star => {
                for i in 0..5 {
                    ...
                    if i == 0 { ctx.move_to(crisp(px, dpr), crisp(py, dpr)); }
                    else { ctx.line_to(crisp(px, dpr), crisp(py, dpr)); }
                }
                ctx.close_path();
                ctx.fill();
            }
...
pub fn metadata() -> PrimitiveMetadata {
    PrimitiveMetadata {
        type_id: "sign", display_name: "Sign", kind: PrimitiveKind::Signal,
        click_behavior: ClickBehavior::TwoPoint, tooltip: "Icon marker", icon: "sign", default_color: "#E91E63",
        factory: |points, color| {
            let (b1, p1) = points.first().copied().unwrap_or((0.0, 0.0));
            let (b2, p2) = points.get(1).copied().unwrap_or((b1 + 2.0, p1));
            Box::new(Sign::new(b1, p1, b2, p2, color))
        },
        supports_text: true,
        has_levels: false,
