...
impl Primitive for Note {
    ...
    fn hit_test(&self, sx: f64, sy: f64, vp: &Viewport, ps: &PriceScale) -> HitTestResult {
        ...
        // Check body area (rectangle between two points)
    ...
    fn render(&self, ctx: &mut dyn RenderContext, is_selected: bool) {
        ...
        // Render text centered inside note
        ...
        match id {
            "content" => {
                if let Some(s) = value.as_string() {
                    ...
                    self.content = s.to_string(); // Keep legacy field in sync
                    return true;
                }
            }
            "font_size" => {
...
pub fn metadata() -> PrimitiveMetadata {
    PrimitiveMetadata {
        ...
        factory: |points, color| { let (b1, p1) = points.first().copied().unwrap_or((0.0, 0.0)); let (b2, p2) = points.get(1).copied().unwrap_or((b1+3.0, p1-20.0)); Box::new(Note::new(b1, p1, b2, p2, color)) },
