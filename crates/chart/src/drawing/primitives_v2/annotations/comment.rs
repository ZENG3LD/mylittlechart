...
impl Primitive for Comment {
    ...
    fn hit_test(&self, sx: f64, sy: f64, vp: &Viewport, ps: &PriceScale) -> HitTestResult {
        ...
        // Check body area (bounding box between points)
        let min_x = x1.min(x2);
        let max_x = x1.max(x2);
        let min_y = y1.min(y2);
        let max_y = y1.max(y2);
    ...
    fn render(&self, ctx: &mut dyn RenderContext, is_selected: bool) {
        ...
        let size = width.min(height).max(20.0); // Minimum size of 20

        // Draw speech bubble icon at center
        ...
        // Render text centered inside the bubble (replaces line indicators)
        ...
        if !label.is_empty() && size > 30.0 {
            ...
            // Draw text indicator (three lines) for small bubbles
...
pub fn metadata() -> PrimitiveMetadata {
    PrimitiveMetadata {
        ...
        factory: |points, color| { let (b1, p1) = points.first().copied().unwrap_or((0.0, 0.0)); let (b2, p2) = points.get(1).copied().unwrap_or((b1+3.0, p1+20.0)); Box::new(Comment::new(b1, p1, b2, p2, color)) },
