...
/// Default render implementation - draws lines between points
///
/// This is used as fallback for primitives that haven't implemented
/// custom rendering yet. Override render() for proper visualization.
pub fn render_default(prim: &dyn Primitive, ctx: &mut dyn RenderContext, is_selected: bool) {
    let data = prim.data();
    ...
    match prim.kind() {
        ...
        _ => {
            // Default: draw lines connecting all points with markers
            if screen_points.len() >= 2 {
                ctx.begin_path();
                let (x0, y0) = screen_points[0];
                ctx.move_to(crisp(x0, dpr), crisp(y0, dpr));
