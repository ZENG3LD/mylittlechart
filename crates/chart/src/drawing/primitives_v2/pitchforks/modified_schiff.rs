...
impl ModifiedSchiff {
    ...
    /// Keeps original ts, moves price to midpoint of P2/P3 (ts in ms, price)
...
impl Primitive for ModifiedSchiff {
    ...
    fn hit_test(
        ...
        // Check each pitchfork line (symmetric ±levels)
    ...
    fn render(&self, ctx: &mut dyn RenderContext, is_selected: bool) {
        ...
        for config in &self.level_configs {
            if config.visible && config.fill_enabled && config.level.abs() > 0.001 {
                ...
                // Start points (at adjusted handle for Modified Schiff)
    ...
    fn style_properties(&self) -> Vec<super::super::config::ConfigProperty> {
        ...
        vec![
            ...
            ConfigProperty::extend_lines(self.extend).with_order(14),
        ]
    }
...
fn create_modified_schiff(points: &[(i64, f64)], color: &str) -> Box<dyn Primitive> {
    ...
    Box::new(ModifiedSchiff::new(ts1, price1, ts2, price2, ts3, price3, color))
}
...
pub fn metadata() -> PrimitiveMetadata {
    PrimitiveMetadata {
        ...
        tooltip: "Modified Schiff Pitchfork",
        icon: "modified_schiff",
