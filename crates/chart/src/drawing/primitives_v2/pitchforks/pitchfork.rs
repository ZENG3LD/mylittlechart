...
impl Pitchfork {
    ...
    /// Get median line direction (ts_delta in ms, price_delta)
    pub fn median_direction(&self) -> (i64, f64) {
        let (mid_ts, mid_price) = self.midpoint();
        (mid_ts - self.ts1, mid_price - self.price1)
    }
}
...
fn create_pitchfork(points: &[(i64, f64)], color: &str) -> Box<dyn Primitive> {
    ...
    Box::new(Pitchfork::new(ts1, price1, ts2, price2, ts3, price3, color))
}
