...
impl SchiffPitchfork {
    ...
    /// Moves the handle halfway to the midpoint of P2 and P3 (ts in ms, price)
    pub fn adjusted_handle(&self) -> (i64, f64) {
        let mid_ts = (self.ts2 + self.ts3) / 2;
        let mid_price = (self.price2 + self.price3) / 2.0;
        (
            (self.ts1 + mid_ts) / 2,
            (self.price1 + mid_price) / 2.0,
        )
    }
    ...
    /// Get the channel half-offset (ts_delta in ms, price_delta)
...
impl Primitive for SchiffPitchfork {
    ...
    fn hit_test(
        ...
        // Check control points (original positions)
        ...
        // Get adjusted handle for Schiff
        let (adj_ts, adj_price) = self.adjusted_handle();
        let adj_b = timestamp_ms_to_bar_f64(bars, adj_ts);
        let adj_x = viewport.bar_to_x_f64(adj_b);
        let adj_y = viewport.price_to_y(adj_price, price_scale.price_min, price_scale.price_max);
        ...
        // Check median line (always present)
        {
            let dist = if self.extend {
                point_to_ray_distance(screen_x, screen_y, adj_x, adj_y, mid_x, mid_y)
            } else {
                point_to_line_distance(screen_x, screen_y, adj_x, adj_y, mid_x, mid_y)
            };
        ...
        // Check each pitchfork line (symmetric ±levels from adjusted handle)
    ...
    fn render(&self, ctx: &mut dyn RenderContext, is_selected: bool) {
        ...
        let (adj_ts, adj_price) = self.adjusted_handle();
        let adj_x = ctx.ts_to_x_ms(adj_ts);
        ...
        // Helper function to draw a pitchfork line with optional gap
        ...
        for config in &self.level_configs {
            if config.visible && config.fill_enabled && config.level.abs() > 0.001 {
                ...
                // Start points (at adjusted handle for Schiff)
                let pos_start_x = adj_x + offset_x * level;
                let pos_start_y = adj_y + offset_y * level;
                let neg_start_x = adj_x + offset_x * (-level);
                let neg_start_y = adj_y + offset_y * (-level);
        ...
        draw_line(ctx, dpr, chart_width, self.extend, adj_x, adj_y, mid_x, mid_y);
        ...
        if let Some(ref text) = self.data.text {
            if !text.content.is_empty() {
                ...
                // - Start: above upper boundary (highest level)
                // - End: below lower boundary (lowest level)
                let (text_start_x, text_start_y, text_end_x, text_end_y) = match text.v_align {
                    super::super::TextAlign::Start | super::super::TextAlign::End => {
                        ...
                        let (upper_level, lower_level) = if visible_levels.is_empty() {
                            ...
                            let max_level_y = adj_y + offset_y * max_level;
                            let min_level_y = adj_y + offset_y * min_level;
                        ...
                        (adj_x + offset_x * level, adj_y + offset_y * level,
                         mid_x + offset_x * level, mid_y + offset_y * level)
                    }
                    super::super::TextAlign::Center => {
                        (adj_x, adj_y, mid_x, mid_y)
                    }
                };
...
fn create_schiff_pitchfork(points: &[(i64, f64)], color: &str) -> Box<dyn Primitive> {
    ...
    Box::new(SchiffPitchfork::new(ts1, price1, ts2, price2, ts3, price3, color))
}
...
pub fn metadata() -> PrimitiveMetadata {
    PrimitiveMetadata {
        ...
        tooltip: "Schiff Pitchfork - adjusted handle point",
        icon: "schiff_pitchfork",
