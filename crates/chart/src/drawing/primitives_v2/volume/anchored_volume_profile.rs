...
impl AnchoredVolumeProfile {
    pub fn new(bar: f64, color: &str) -> Self {
        Self {
            ...
            anchor_bar: bar, rows: 24, show_poc: true,
        }
    }
}
...
impl Primitive for AnchoredVolumeProfile {
    ...
    fn points(&self) -> Vec<(f64, f64)> { vec![(self.anchor_bar, 0.0)] }
    fn set_points(&mut self, pts: &[(f64, f64)]) { if let Some(&(b, _)) = pts.first() { self.anchor_bar = b; } }
    fn translate(&mut self, bd: f64, _pd: f64) { self.anchor_bar += bd; }
    fn move_control_point(&mut self, pt: ControlPointType, bar: f64, _price: f64) {
        if matches!(pt, ControlPointType::Point1 | ControlPointType::Move) { self.anchor_bar = bar; }
    }
    fn hit_test(&self, sx: f64, _sy: f64, vp: &Viewport, _ps: &PriceScale) -> HitTestResult {
        let x = vp.bar_to_x_f64(self.anchor_bar);
    ...
    fn control_points(&self, vp: &Viewport, ps: &PriceScale) -> Vec<ControlPoint> {
        ...
        vec![ControlPoint::point1(vp.bar_to_x_f64(self.anchor_bar), cy)]
    }
...
pub fn metadata() -> PrimitiveMetadata {
    PrimitiveMetadata {
        ...
        factory: |points, color| { let (b, _) = points.first().copied().unwrap_or((0.0, 0.0)); Box::new(AnchoredVolumeProfile::new(b, color)) },
        supports_text: true,
        has_levels: false,
