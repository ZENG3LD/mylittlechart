...
impl Primitive for Path {
    fn type_id(&self) -> &'static str { "path" }
    ...
    fn hit_test(&self, screen_x: f64, screen_y: f64, bars: &[Bar], viewport: &Viewport, price_scale: &PriceScale) -> HitTestResult {
        ...
        if self.closed && screen_pts.len() > 2 {
            let (x1, y1) = screen_pts[screen_pts.len() - 1];
            let (x2, y2) = screen_pts[0];
            if point_to_line_distance(screen_x, screen_y, x1, y1, x2, y2) < HIT_TOLERANCE {
                return HitTestResult::Body;
            }
        }
    ...
    fn render(&self, ctx: &mut dyn RenderContext, is_selected: bool) {
        ...
        ctx.begin_path();
        if self.smooth && screen_pts.len() >= 3 {
            ctx.move_to(screen_pts[0].0, screen_pts[0].1);
            for i in 1..screen_pts.len() - 1 {
                let (x0, y0) = screen_pts[i - 1];
                let (x1, y1) = screen_pts[i];
                let (x2, y2) = screen_pts[i + 1];
                let end_x = (x1 + x2) / 2.0;
                let end_y = (y1 + y2) / 2.0;
                if i == 1 { ctx.line_to((x0 + x1) / 2.0, (y0 + y1) / 2.0); }
                ctx.quadratic_curve_to(x1, y1, end_x, end_y);
            }
...
fn create_path(points: &[(i64, f64)], color: &str) -> Box<dyn Primitive> {
    Box::new(Path::new(points.to_vec(), color))
}

pub fn metadata() -> PrimitiveMetadata {
    PrimitiveMetadata {
        type_id: "path", display_name: "Path", kind: PrimitiveKind::Shape,
        click_behavior: ClickBehavior::MultiPoint(2),
        tooltip: "Draw a freeform path (double-click to finish)",
        icon: "path", default_color: "#009688",
        factory: create_path,
