...
impl InfoLine {
    pub fn new(ts1: i64, price1: f64, ts2: i64, price2: f64, color: &str) -> Self {
        Self {
            ...
            show_price_diff: true, show_percent: true, show_bars: true,
        }
    }

    pub fn price_diff(&self) -> f64 { self.price2 - self.price1 }

    pub fn percent_change(&self) -> f64 {
        if self.price1.abs() < 1e-10 { 0.0 }
        else { ((self.price2 - self.price1) / self.price1) * 100.0 }
    }

    /// Approximate bar count from timestamp delta (uses bars slice if available via render)
    pub fn bar_count_from_bars(&self, bars: &[Bar]) -> i64 {
        let interval_ms = bar_interval_seconds(bars) * 1000;
        if interval_ms == 0 { return 0; }
        ((self.ts2 - self.ts1).abs() / interval_ms).max(0)
    }

    pub fn info_text_with_bars(&self, bars: &[Bar]) -> String {
        ...
        if self.show_bars {
            let bars_count = self.bar_count_from_bars(bars);
            parts.push(format!("{} bars", bars_count));
        }
        parts.join(" ")
    }
}
...
impl Primitive for InfoLine {
    fn type_id(&self) -> &'static str { "info_line" }
    ...
    fn render(&self, ctx: &mut dyn RenderContext, is_selected: bool) {
        ...
        let info_text = {
            ...
            if self.show_bars {
                // Use ts delta; bars not available here
                let delta_ms = (self.ts2 - self.ts1).abs();
                parts.push(format!("{} ms", delta_ms));
            }
            parts.join(" ")
        };
        ...
        if !info_text.is_empty() {
            ...
            let padding = 6.0; let bg_width = text_width + padding * 2.0; let bg_height = 20.0;
...
fn create_info_line(points: &[(i64, f64)], color: &str) -> Box<dyn Primitive> {
    let (ts1, price1) = points.first().copied().unwrap_or((0, 0.0));
    let (ts2, price2) = points.get(1).copied().unwrap_or((ts1, price1));
    Box::new(InfoLine::new(ts1, price1, ts2, price2, color))
}

pub fn metadata() -> PrimitiveMetadata {
    PrimitiveMetadata {
        type_id: "info_line", display_name: "Info Line", kind: PrimitiveKind::Line,
        click_behavior: ClickBehavior::TwoPoint,
        tooltip: "Draw a line showing price difference, percentage, and bars",
        icon: "info_line", default_color: "#9C27B0", factory: create_info_line,
