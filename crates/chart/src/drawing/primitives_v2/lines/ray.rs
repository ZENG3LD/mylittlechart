...
impl Primitive for Ray {
    fn type_id(&self) -> &'static str { "ray" }
    ...
    fn hit_test(&self, screen_x: f64, screen_y: f64, bars: &[Bar], viewport: &Viewport, price_scale: &PriceScale) -> HitTestResult {
        ...
        } else { (viewport.chart_width + 100.0, y1) };
    ...
    fn render(&self, ctx: &mut dyn RenderContext, is_selected: bool) {
        ...
        let (extend_x, extend_y) = if base_len > 0.001 {
            let nx = dx / base_len; let ny = dy / base_len;
            let extend_dist = ctx.chart_width() + ctx.chart_height();
            (x1 + nx * extend_dist, y1 + ny * extend_dist)
        } else { (ctx.chart_width(), y1) };
        ...
        if needs_gap && base_len > 0.001 {
            ...
            if t_start > 0.001 {
                let gap_x = x1 + dx * t_start; let gap_y = y1 + dy * t_start;
                ctx.move_to(crisp(x1, dpr), crisp(y1, dpr));
                ctx.line_to(crisp(gap_x, dpr), crisp(gap_y, dpr));
            }
            let gap_x2 = x1 + dx * t_end; let gap_y2 = y1 + dy * t_end;
...
fn create_ray(points: &[(i64, f64)], color: &str) -> Box<dyn Primitive> {
    let (ts1, price1) = points.first().copied().unwrap_or((0, 0.0));
    let (ts2, price2) = points.get(1).copied().unwrap_or((ts1, price1));
    Box::new(Ray::new(ts1, price1, ts2, price2, color))
}

pub fn metadata() -> PrimitiveMetadata {
    PrimitiveMetadata {
        type_id: "ray", display_name: "Ray", kind: PrimitiveKind::Line,
        click_behavior: ClickBehavior::TwoPoint, tooltip: "Draw a ray extending to the right",
        icon: "ray", default_color: "#2196F3", factory: create_ray,
