...
impl Primitive for ExtendedLine {
    ...
    fn hit_test(
        ...
        // For extended line, extend both directions
        let dx = x2 - x1;
        let dy = y2 - y1;

        // Extend to viewport edges
        let left_x = -100.0;
        let right_x = viewport.chart_width + 100.0;

        let (ext_x1, ext_y1, ext_x2, ext_y2) = if dx.abs() > 0.0001 {
            let slope = dy / dx;
            let ext_y1 = y1 + slope * (left_x - x1);
            let ext_y2 = y1 + slope * (right_x - x1);
            (left_x, ext_y1, right_x, ext_y2)
        } else {
    ...
    fn render(&self, ctx: &mut dyn RenderContext, is_selected: bool) {
        ...
        // Calculate extension to both edges
        let dx = x2 - x1;
        let dy = y2 - y1;
        ...
        if needs_gap && base_len > 0.001 {
            ...
            // Text position along p1-p2 segment (0..1)
            ...
            // Gap coordinates
            let gap_x1 = x1 + dx * t_gap_start;
            let gap_y1 = y1 + dy * t_gap_start;
            let gap_x2 = x1 + dx * t_gap_end;
            let gap_y2 = y1 + dy * t_gap_end;

            // Draw left segment (extended left to gap start)
            ctx.move_to(crisp(0.0, dpr), crisp(left_y, dpr));
            ctx.line_to(crisp(gap_x1, dpr), crisp(gap_y1, dpr));

            // Draw right segment (gap end to extended right)
            ctx.move_to(crisp(gap_x2, dpr), crisp(gap_y2, dpr));
            ctx.line_to(crisp(ctx.chart_width(), dpr), crisp(right_y, dpr));
        } else {
            // No gap - draw full extended line
            ctx.move_to(crisp(0.0, dpr), crisp(left_y, dpr));
            ctx.line_to(crisp(ctx.chart_width(), dpr), crisp(right_y, dpr));
        }
...
pub fn metadata() -> PrimitiveMetadata {
    PrimitiveMetadata {
        type_id: "extended_line",
        display_name: "Extended Line",
        kind: PrimitiveKind::Line,
        click_behavior: ClickBehavior::TwoPoint,
        tooltip: "Draw a line extending in both directions",
        icon: "extended_line",
        default_color: "#2196F3",
        factory: create_extended_line,
        supports_text: true,
        has_levels: false,
