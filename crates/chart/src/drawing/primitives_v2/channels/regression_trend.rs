...
impl Primitive for RegressionTrend {
    ...
    fn hit_test(
        ...
        // Check center line
        if self.show_center && point_to_line_distance(screen_x, screen_y, x1, y1, x2, y2) < HIT_TOLERANCE {
            return HitTestResult::Body;
        }

        // Check upper band
        if point_to_line_distance(screen_x, screen_y, x1, upper_y1, x2, upper_y2) < HIT_TOLERANCE {
            return HitTestResult::Body;
        }

        // Check lower band
        if point_to_line_distance(screen_x, screen_y, x1, lower_y1, x2, lower_y2) < HIT_TOLERANCE {
            return HitTestResult::Body;
        }
    ...
    fn render(&self, ctx: &mut dyn RenderContext, is_selected: bool) {
        ...
        if let Some(ref text) = self.data.text {
            if !text.content.is_empty() {
                ...
                // - Center: on center (regression) line
                // - Start: on upper band
                // - End: on lower band
