...
pub struct PriceProjection {
    ...
    #[serde(default)] pub ts3: i64, pub price3: f64,
...
impl PriceProjection {
    pub fn new(ts1: i64, price1: f64, ts2: i64, price2: f64, ts3: i64, price3: f64, color: &str) -> Self {
        Self {
            ...
            ts1, price1, ts2, price2, ts3, price3, show_percentage: true,
        }
    }
}
...
impl Primitive for PriceProjection {
    ...
    fn points(&self) -> Vec<(i64, f64)> { vec![(self.ts1, self.price1), (self.ts2, self.price2), (self.ts3, self.price3)] }
    fn set_points(&mut self, pts: &[(i64, f64)]) {
        ...
        if let Some(&(t, p)) = pts.get(2) { self.ts3 = t; self.price3 = p; }
    }
    ...
    fn hit_test(&self, sx: f64, sy: f64, bars: &[Bar], vp: &Viewport, ps: &PriceScale) -> HitTestResult {
        let pts = [(self.ts1, self.price1), (self.ts2, self.price2), (self.ts3, self.price3)];
        let screen: Vec<_> = pts.iter().map(|(t, p)| {
            let b = timestamp_ms_to_bar_f64(bars, *t);
    ...
    fn control_points(&self, bars: &[Bar], vp: &Viewport, ps: &PriceScale) -> Vec<ControlPoint> {
        ...
        vec![
            ...
            ControlPoint::point3(vp.bar_to_x_f64(b3), vp.price_to_y(self.price3, ps.price_min, ps.price_max)),
        ]
    }
...
pub fn metadata() -> PrimitiveMetadata {
    PrimitiveMetadata {
        ...
        factory: |points, color| {
            ...
            let (t2, p2) = points.get(1).copied().unwrap_or((t1 + 3_600_000, p1 + 5.0));
            let (t3, p3) = points.get(2).copied().unwrap_or((t2 + 3_600_000, p2 + 5.0));
            Box::new(PriceProjection::new(t1, p1, t2, p2, t3, p3, color))
        },
        supports_text: true,
        has_levels: false,
