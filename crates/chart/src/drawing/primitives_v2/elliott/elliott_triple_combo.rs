...
pub struct ElliottTripleCombo {
    ...
    /// Variable number of points for WXYXZ pattern
...
impl ElliottTripleCombo {
    ...
    /// For WXYXZ: positions 0=W, 3=X, 4=Y, 6=X2, 7=Z (others empty for subwaves)
    pub fn wave_labels(&self) -> [&'static str; 9] {
        let combo = self.degree.combo_labels();
        // W, X, Y, X2, Z at key positions
...
impl Primitive for ElliottTripleCombo {
    ...
    fn move_control_point(&mut self, pt: ControlPointType, bar: f64, price: f64) {
        match pt {
            ControlPointType::Index(i) if (i as usize) < self.points.len() => self.points[i as usize] = (bar, price),
            ControlPointType::Move if !self.points.is_empty() => { let bd = bar - self.points[0].0; let pd = price - self.points[0].1; self.translate(bd, pd); }
            _ => {}
        }
    }
