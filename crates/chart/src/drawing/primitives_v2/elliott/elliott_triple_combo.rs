...
pub struct ElliottTripleCombo {
    ...
    /// Variable number of points for WXYXZ pattern. Each is (timestamp_ms, price).
...
impl ElliottTripleCombo {
    ...
    pub fn wave_labels(&self) -> [&'static str; 9] {
...
impl Primitive for ElliottTripleCombo {
    fn type_id(&self) -> &'static str { "elliott_triple_combo" }
    fn display_name(&self) -> &str { &self.data.display_name }
    fn kind(&self) -> PrimitiveKind { PrimitiveKind::Pattern }
    fn click_behavior(&self) -> ClickBehavior { ClickBehavior::MultiPoint(9) }
    ...
    fn move_control_point(&mut self, pt: ControlPointType, ts_ms: i64, price: f64) {
        match pt {
            ControlPointType::Index(i) if (i as usize) < self.points.len() => self.points[i as usize] = (ts_ms, price),
            ControlPointType::Move if !self.points.is_empty() => { let td = ts_ms - self.points[0].0; let pd = price - self.points[0].1; self.translate(td, pd); }
            _ => {}
        }
    }
    ...
    fn render(&self, ctx: &mut dyn RenderContext, is_selected: bool) {
        ...
        if self.show_labels {
            ...
            for (i, (x, y)) in screen.iter().enumerate() {
                if i >= labels.len() || labels[i].is_empty() { continue; }
                let label = labels[i];
                let offset = if i > 0 && *y < screen[i - 1].1 { -self.label_style.offset_y } else { self.label_style.offset_y };
...
pub fn metadata() -> PrimitiveMetadata {
    PrimitiveMetadata {
        type_id: "elliott_triple_combo", display_name: "Elliott Triple Combo", kind: PrimitiveKind::Pattern,
        click_behavior: ClickBehavior::MultiPoint(9), tooltip: "WXYXZ triple combination", icon: "elliott_triple_combo", default_color: "#673AB7",
