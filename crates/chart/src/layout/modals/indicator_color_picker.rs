//! Indicator color picker popup renderer.
...
/// Render color picker popup for indicator settings (L1 or L2 based on state)
pub fn render_indicator_color_picker_popup(
    ctx: &mut dyn RenderContext,
    indicator_state: &crate::ui::modal_settings::IndicatorSettingsState,
    ...
    let origin = indicator_state.color_picker.origin;
    let level = indicator_state.color_picker.level;

    let result = match level {
        ColorPickerLevel::L1 => {
            let config = indicator_state.color_picker.l1_config();
            let hovered = indicator_state.color_picker.hovered_swatch_str();
        ...
        ColorPickerLevel::L2 => {
            let config = indicator_state.color_picker.l2_config();
            let hovered_area = indicator_state.color_picker.hovered_area;
    ...
    let layer_id = ZLayer::ColorPicker.push_named(input_coordinator, "color_picker_indicator");

    if let Some(ref l1) = result.l1_result {
        let popup_rect = &l1.popup_rect;
        input_coordinator.register_on_layer(
            "color_picker_indicator:popup",
        ...
        for (idx, (_, swatch_rect)) in l1.swatch_rects.iter().enumerate() {
            input_coordinator.register_on_layer(
                format!("color_picker_indicator:swatch:{}", idx),
        ...
        if let Some(ref slider_rect) = l1.opacity_slider_rect {
            input_coordinator.register_on_layer(
                "color_picker_indicator:opacity_slider",
        ...
        if let Some(ref toggle_rect) = l1.opacity_toggle_rect {
            input_coordinator.register_on_layer(
                "color_picker_indicator:opacity_toggle",
        ...
        input_coordinator.register_on_layer(
            "color_picker_indicator:sv_square",
        ...
        input_coordinator.register_on_layer(
            "color_picker_indicator:hue_bar",
        ...
        input_coordinator.register_on_layer(
            "color_picker_indicator:hex_input",
