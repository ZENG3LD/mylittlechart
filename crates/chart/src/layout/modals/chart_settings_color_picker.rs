//! Chart settings color picker popup renderer.
...
/// Render color picker popup for chart settings (L1 or L2 based on state)
pub fn render_chart_settings_color_picker_popup(
    ctx: &mut dyn RenderContext,
    chart_settings_state: &crate::ui::modal_settings::ChartSettingsState,
    ...
    let origin = chart_settings_state.color_picker.origin;
    let level = chart_settings_state.color_picker.level;

    let result = match level {
        ColorPickerLevel::L1 => {
            let config = chart_settings_state.color_picker.l1_config();
            let hovered = chart_settings_state.color_picker.hovered_swatch_str();
        ...
        ColorPickerLevel::L2 => {
            let config = chart_settings_state.color_picker.l2_config();
            let hovered_area = chart_settings_state.color_picker.hovered_area;
    ...
    let layer_id = ZLayer::ColorPicker.push_named(input_coordinator, "color_picker_chart");

    if let Some(ref l1) = result.l1_result {
        let popup_rect = &l1.popup_rect;
        input_coordinator.register_on_layer(
            "color_picker_chart:popup",
        ...
        for (idx, (_, swatch_rect)) in l1.swatch_rects.iter().enumerate() {
            input_coordinator.register_on_layer(
                format!("color_picker_chart:swatch:{}", idx),
        ...
        if let Some(ref toggle_rect) = l1.opacity_toggle_rect {
            input_coordinator.register_on_layer(
                "color_picker_chart:opacity_toggle",
